...
export async function initDatabaseFromText(text: string) {
  const jsonList = text.split('\n').filter(Boolean).map(row => JSON.parse(row) as VideoInfoJSON)
...
export function unparseInfoJSON(originalJsonText: string, keyMoments: Record<string, KeyMoment[]>) {
  return originalJsonText.split('\n').filter(Boolean).map(row => JSON.parse(row) as VideoInfoJSON).map(json => {
    const newJson = { ...json, "Key Moment": keyMoments[json.MID].map(unparseKeyMomentJSON) }
    return newJson
...
async function parseSentencesJSON(url: string): Promise<Record<string, Sentence[]>> {
  return new Promise((resolve, reject) => {
    ...
    Papa.parse<SentenceJSON>(url, {
      ...
      complete: (result) => {
        ...
        const sentences = result.data.reduce((acc, s) => {
          if (acc[s.MID] == null) {
            acc[s.MID] = []
          }
